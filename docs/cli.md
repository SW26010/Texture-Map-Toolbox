...
| `editor` | `texture_map_toolbox.cli.editor` | Oklch 状态曲线编辑器 |
...
3. 如果需要绘图展示，调用 `texture_map_toolbox.gui.luma_plots`。
4. 如果仍希望走命令行协议，统一调用 `texture_map_toolbox.__main__.main(argv)`，并优先使用 request/result JSON 交换数据。